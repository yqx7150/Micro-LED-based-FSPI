...
# 加载mask数组
random_mask = scio.loadmat(f'mask/random_mask_{fnumb}.mat')['mask']
# random_mask = scio.loadmat(f'/home/b110/code1/xzz/ncsn_test/ncsnpp_ct_small_versioin2/random2D_R4.mat')['mask']
random_mask = torch.from_numpy(random_mask)
random_mask = random_mask.to(torch.float32).to('cuda')
...
#     f_value_r = I1-I3
#     f_value_i = I2-I4
#     f_value = torch.stack([f_value_r,f_value_i],dim=0)
#     f_value = f_value[:,30:]
#     f_value2 = torch.flip(f_value[:,1:],dims=[1])
#     f_value2[1] = -f_value2[1]
#     f_value = torch.cat([f_value2,f_value],dim=1)
#     image_k = torch.reshape(f_value,[2,61,61])
#     image_k = image_k.permute(1,2,0)
#     image_k = torch.unsqueeze(image_k,0)
    
#     image = torch.irfft(image_k,signal_ndim=2,signal_sizes=(61,61),onesided=False)
...
def samples(img,device):
    ...
    for i in range(7564):
        #fp = sio.loadmat("/home/liuqg/LX/base_image_128X128/" + str(i+1) + ".mat")
        fp = sio.loadmat("/home/lqg/LX/real_construction_data/z6.jpg")
        p = fp['p1']
        p = np.float64(p)
        p = torch.from_numpy(p).to(device)
        p2 = p*img
...
def get_pc_sampler(sde, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model):
    ...
    def dc(x_mean_k, y_k):
      ...
      # if i % 10 != 0: return
      # if i % 4 != 0: return
      # x_mean_k = x_mean_k * (1.0-random_mask) + y_k * random_mask
      # 5%
      if flag == 5:
        if y_k_centerX < 28:
          x_mean_k[img_center-y_k_centerX:img_center+y_k_centerX,img_center-y_k_centerX:img_center+y_k_centerX] = y_k[:,y_k_centerY-y_k_centerX:y_k_centerY+y_k_centerX]
        else:
          if img_size == 128:
            x_mean_k[img_center-23:img_center+23,img_center-23:img_center+23] = y_k[y_k_centerX-23:y_k_centerX+23,y_k_centerY-23:y_k_centerY+23]
          else:
            x_mean_k[img_center-28:img_center+28,img_center-28:img_center+28] = y_k[y_k_centerX-28:y_k_centerX+28,y_k_centerY-28:y_k_centerY+28]
      # 3%
      elif flag == 3:
        if y_k_centerX < 22:
          x_mean_k[img_center-y_k_centerX:img_center+y_k_centerX,img_center-y_k_centerY:img_center+y_k_centerY] = y_k
        else:
          if img_size == 128:
            x_mean_k[img_center-11:img_center+11,img_center-11:img_center+11] = y_k[y_k_centerX-11:y_k_centerX+11,y_k_centerY-11:y_k_centerY+11]
          else:
            x_mean_k[img_center-22:img_center+22,img_center-22:img_center+22] = y_k[y_k_centerX-22:y_k_centerX+22,y_k_centerY-22:y_k_centerY+22]
      # 1%
      else:
        if y_k_centerX < 13:
          x_mean_k[img_center-y_k_centerX:img_center+y_k_centerX,img_center-y_k_centerY:img_center+y_k_centerY] = y_k
          ...
          if img_size == 128:
            x_mean_k[img_center-7:img_center+7,img_center-7:img_center+7] = y_k[y_k_centerX-7:y_k_centerX+7,y_k_centerY-7:y_k_centerY+7]
          else:
            x_mean_k[img_center-13:img_center+13,img_center-13:img_center+13] = y_k[y_k_centerX-13:y_k_centerX+13,y_k_centerY-13:y_k_centerY+13]
            # x_mean_k[img_center-10:img_center+10,img_center-10:img_center+10] = y_k[y_k_centerX-10:y_k_centerX+10,y_k_centerY-10:y_k_centerY+10]
            # x_mean_k[img_center-16:img_center+16,img_center-16:img_center+16] = y_k[y_k_centerX-16:y_k_centerX+16,y_k_centerY-16:y_k_centerY+16]
          # x_mean_k[128-11:128+11,128-11:128+11] = y_k[y_k_centerX-11:y_k_centerX+11,y_k_centerY-11:y_k_centerY+11]
          # x_mean_k[128-9:128+9,128-9:128+9] = y_k[y_k_centerX-9:y_k_centerX+9,y_k_centerY-9:y_k_centerY+9]
    ...
    #导入图片
    # x_mean = torch.full([1,1,256,256],0.5,dtype=torch.float64)
    #随机种子   22
    torch.manual_seed(0)
    x_mean = torch.rand([1,1,img_size,img_size],dtype=torch.float32)
    x_mean = x_mean.to(device).type(torch.cuda.FloatTensor)
    
   
    y_k = skimage.io.imread('/home/a604/code/lh/microled/1.jpeg',as_gray=True)
    y_k = skimage.transform.resize(y_k,(33,33)
    y_k = y_k/np.max(y_k)
    y_k = np.fft.fftshift(np.fft.fft2(y_k))
    y_k = torch.from_numpy(y_k)
    y_k = y_k*x
    
    y_k_centerX = y_k.shape[0] // 2
    y_k_centerY = y_k.shape[1] // 2
    y_k = y_k.to(device)
    ...
    for i in range(1000):
      ...
      x_mean_k = squeeze_fft(x_mean)
      dc(x_mean_k, y_k)
      x_mean = ifft_unsqueeze(x_mean_k)
      # P
      ...
      skimage.io.imsave(f"reconstruct/led/{i}.bmp", img_as_ubyte(out))
      ...
      # out = np.squeeze(out,0)
      # out = np.squeeze(out,0)
      # skimage.io.imsave(f"reconstruct/cat/simulate/128/{flag}/{i}.bmp", img_as_ubyte(out))
      # skimage.io.imsave(f"reconstruct/coin/my/practical/obverse/{x}/{i}.bmp", img_as_ubyte(out))
      # skimage.io.imsave(f"reconstruct/1/{i}.bmp", img_as_ubyte(out))
      print(i)
  return pc_sampler
'''
